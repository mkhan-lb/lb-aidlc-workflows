...
> adds service scopes with 12 or 15 configured stages across all seven harnesses.
> Uses AWS 2.8.1 native releases: consumers need no Bun or Node.js. Follow the
> distribution guide for verified LB archives and safe upgrades. Maintainers build
> with `bun scripts/package-lb.ts`; generated `dist-release/` remains stock.
> Downstream versions and changes live in [LB_CHANGELOG.md](LB_CHANGELOG.md).
